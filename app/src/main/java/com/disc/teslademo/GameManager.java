...
public class GameManager extends FragmentActivity
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        currentPar = course.getABasketPar(0);
...
//                    if (checkInput(readMessage))
    ...
    public void onMapReady(GoogleMap map) {
        ...
        if ((lastLocation != null)) {
            ...
            map.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(MainActivity.course.getATpadLatitude(0), MainActivity.course.getATpadLongitude(0)), 18f));
        }
    ...
    private void updateMapDrawings() {
        ...
        for (int i = 0; i < MainActivity.course.getBasketLatitudes().size(); i++) {
            gameMap.getMap().addMarker(new MarkerOptions()
                    ...
                    .position(new LatLng(MainActivity.course.getABasketLatitude(i), MainActivity.course.getABasketLongitude(i))));

            gameMap.getMap().addMarker(new MarkerOptions()
                    //.icon(BitmapDescriptorFactory.fromResource(R.drawable.)) // basket
                    .anchor(0.0f, 1.0f) // Anchors the marker on the bottom left
                    .position(new LatLng(MainActivity.course.getATpadLatitude(i), MainActivity.course.getATpadLongitude(i))));
        }
    }
